...
- 更适合后续继续接入 `Invero`、货币插件、权限插件等服内系统
...
- 执行 `/mmmland menu`
- 通过 `Invero` 主菜单中的“领地”入口按钮进入

当前 `Invero` 主要负责入口，不负责完整业务逻辑。
...
## 8. 为什么当前不是“纯 Invero 菜单插件”

因为 `Invero` 更适合做菜单展示和基础交互，不适合承载复杂业务状态。

例如下面这些逻辑，用纯 `Invero` 会变得很难维护：
...
- 当前创建确认只针对“创建领地”，扩张和缩小还不是聊天确认流程
- 当前菜单仍然是本插件自己的 Bukkit GUI，不是完整交由 `Invero` 驱动
...
后续继续优化时，建议围绕“交互体验、配置灵活度、管理能力、与 Invero 更深整合”四个方向推进。
