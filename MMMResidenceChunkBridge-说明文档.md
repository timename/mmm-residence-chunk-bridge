# mmm-residence-chunk-bridge 说明文档

## 1. 插件定位

`mmm-residence-chunk-bridge` 是一个依赖 `Residence 6.0.0.1` 的上层业务插件。
...
- `mmm-residence-chunk-bridge` 负责把玩家的操作转译成符合本服规则的 `Residence` 调用
...
- `currency-tiers` 表示第几个领地对应的货币类型和金额；`vault` 是金币，`custom` 是萌萌贝壳
...
当前 `0.24.0` 属于功能新增与界面优化版本，主要调整了：
- 领地操作菜单重新排版，常用操作按功能分组并保持中间区域对称。
- 删除领地按钮描述改为“不可逆删除领地”，避免向玩家暴露托管记录或底层 Residence 细节。
- 新增 `invero/领地操作.yml` 菜单模板，可复制到 Invero workspace 后按服务器需要自行定制。
- 领地扣款流程改为异步等待 MMMVaultSync 的 CompletableFuture 完成，不再在 Bukkit 主线程阻塞 .get()。
- 修复自管货币扣款实际成功后，主线程阻塞导致 Future 无法完成、本插件误报余额不足且领地未创建的问题。
- 扣款改为串行确认：自管货币失败时不再继续扣默认货币，减少部分扣款风险。
- 修复外部经济插件异步落账时，货币已经实际扣除但本插件仍立即提示余额不足、导致领地未创建的问题。
- 扣款接口返回失败时，会短时间轮询确认余额是否已经减少；确认已扣款后继续执行圈地流程。

- 我的领地列表图标整体向右下移动一格
- 修复外部货币接口实际扣款但返回失败时仍提示余额不足的问题
- 创建领地价格支持按第 N 个领地分别配置货币类型和金额
- 默认创建价格：第 1 个免费，第 2 个 1000 金币，第 3 个 5000 金币，第 4 个 100 萌萌贝壳，第 5 个 500 萌萌贝壳，第 6 个 1000 萌萌贝壳
- 旧配置文件会自动补齐缺失的配置项和语言项
- 我的领地列表图标改为靠中间排列
...
- 当前菜单默认是插件自己的 Bukkit GUI；仓库内同时提供 Invero 模板，方便服主自行定制菜单
